...
(* Differentiation and batching of the FFT family. Gradients of rfft/irfft
   compositions are validated against central finite differences; the pulls are
   additionally pinned against the DFT-definition transpose, whose Hermitian
   double-counting mask differs between even and odd lengths. vmap is checked
   against the loop oracle for all four transforms. *)
...
let test_grad_truncated_spectrum () =
  ...
  check_grad ~msg:"irfft ~n:4 (rfft x), 8 samples"
    (fun x -> Nx.irfft ~n:4 (Nx.rfft x))
    (x8 ())

let test_grad_2d () =
  check_grad ~msg:"irfft2(rfft2 x)"
    (fun x -> Nx.irfft2 ~s:[ 3; 4 ] (Nx.rfft2 x))
    (mat64 3 4
       [| 0.5; -1.2; 2.1; 1.7; -0.4; 0.9; 0.2; 1.3; -0.7; 0.8; -1.6; 0.4 |])
...
let test_grad_spectral_energy () =
  ...
  check_grad ~msg:"filtered spectral energy"
    (fun x ->
      let y = Nx.irfft ~n:8 (Nx.mul (Nx.rfft x) h) in
      Nx.mul y y)
    (x8 ())

(* Pulls against the DFT-definition transpose. With cotangents carrying dL/dRe +
   i dL/dIm, the pull of [rfft] at bin j is sum_k Re (ct_k e^{+2 pi i j k / n})
   over the half spectrum, and the pull of [irfft] at bin k is factor_k / n *
...
let test_rfft_pull_even () =
  let ct = ct4 () in
  let _, g = Rune.vjp' Nx.rfft (x4 ()) ct in
...
let test_rfft_pull_odd () =
  ...
  let _, g = Rune.vjp' Nx.rfft x ct in
...
let test_irfft_pull_even () =
  ...
  let _, g = Rune.vjp' (fun y -> Nx.irfft ~n:4 y) y w in
...
let test_irfft_pull_odd () =
  ...
  let _, g = Rune.vjp' (fun y -> Nx.irfft ~n:5 y) y w in
  check_carr ~msg:"irfft pull, n=5" (irfft_pull_reference 5 3 (to_arr w)) g

(* Forward mode *)

let test_jvp_roundtrip_even () =
  check_jvp ~msg:"irfft(rfft x), n=8" (fun x -> Nx.irfft (Nx.rfft x)) (x8 ())
...
let test_jvp_rfft_is_linear () =
  ...
  let _, dy = Rune.jvp' Nx.rfft (x8 ()) v in
  check_carr ~msg:"jvp rfft = rfft v" (to_carr (Nx.rfft v)) dy

let test_jvp_irfft_is_linear () =
  let y = ct4 () in
  let v = cvec [| (0.6, 0.2); (-0.8, 1.3); (0.4, -0.5) |] in
  let _, dy = Rune.jvp' (fun y -> Nx.irfft ~n:5 y) y v in
  check_arr ~eps:1e-10 ~msg:"jvp irfft = irfft v" (to_arr (Nx.irfft ~n:5 v)) dy

let test_jvp_vjp_consistency () =
  (* <w, jvp_v f x> = <vjp_w f x, v> for the filtered spectral pipeline. *)
  let h = h5 () in
  let f x = Nx.irfft ~n:8 (Nx.mul (Nx.rfft x) h) in
  let x = x8 () in
  let v = vec64 [| 0.3; -0.9; 1.4; 0.1; 2.0; -0.6; 0.8; -1.7 |] in
...
let test_vmap_rfft () = check_vmap_c ~msg:"rfft" (fun x -> Nx.rfft x) (xs ())

let test_vmap_irfft () =
  check_vmap_r ~msg:"irfft" (fun z -> Nx.irfft ~n:6 z) (zs ())
...
let test_vmap_rfft_non_last_axis () =
  ...
  in
  check_vmap_c ~msg:"rfft axis 0" (fun m -> Nx.rfft ~axis:0 m) x
...
let test_vmap_non_leading_batch_axis () =
  ...
  let x =
    Nx.create f64 [| 5; 3 |]
      (Array.init 15 (fun i -> float_of_int ((i * 7 mod 13) - 6) /. 4.0))
  in
  let looped = loop_map (fun r -> Nx.rfft r) (Nx.transpose x) in
  check_carr ~msg:"rfft over axis-1 lanes" (to_carr looped)
    (Rune.vmap' ~in_axis:1 (fun r -> Nx.rfft r) x)
...
let test_vmap_of_grad () =
  ...
  in
  let energy x =
    let y = Nx.irfft ~n:5 (Nx.rfft x) in
    Nx.sum (Nx.mul y y)
  in
...
let test_jit_rfft_refused () =
  let g = Rune.jit' (fun x -> Nx.rfft x) in

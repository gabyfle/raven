...
let rcase name n =
  ...
  Thumper.bench name (fun () -> Nx.rfft input)

let icase name n =
  let spectrum = Nx.rfft (Nx.rand Nx.Float64 [| n |]) in
  Thumper.bench name (fun () -> Nx.irfft ~n spectrum)

let icase_batched name lines n =
  let spectrum = Nx.rfft (Nx.rand Nx.Float64 [| lines; n |]) in
  Thumper.bench name (fun () -> Nx.irfft ~n spectrum)

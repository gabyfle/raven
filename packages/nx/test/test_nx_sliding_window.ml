...
let test_overlapping_view_rejects_writes () =
  let x = Nx.create Nx.float32 [| 5 |] [| 0.; 1.; 2.; 3.; 4. |] in
  let windows = Nx.sliding_window_view ~window:3 x in
  let source = Nx.zeros Nx.float32 (Nx.shape windows) in
...
let test_disjoint_view_accepts_writes () =
  ...
  let blocks = Nx.sliding_window_view ~window:2 ~step:2 x in
  Nx.blit (Nx.ones Nx.float32 (Nx.shape blocks)) blocks;
  check_data "disjoint blocks take the write" [| 1.; 1.; 1.; 1.; 1.; 1. |] x
...
let suite =
  [
    ...
    group "views"
      [
        test "shares storage" test_view_shares_storage;
        test "overlapping rejects writes" test_overlapping_view_rejects_writes;
        test "disjoint accepts writes" test_disjoint_view_accepts_writes;
      ];
